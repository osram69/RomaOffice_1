...
export async function generateMetadata({params}:Props):Promise<Metadata>{const{slug}=await params;const{key}=resolvePath(["en",slug]);const p=pages[key];const canonical=`/${key}`;const itKey=reverseRouteMap[key]||"404.html";const it=itKey==="index.html"?"/":`/${itKey}`;return{title:`${p.title} | Roma Office Sharing`,description:p.description,keywords:["Legal Domiciliation Rome","Registered Office Rome","Postal Domiciliation Rome","Virtual Office Rome","Furnished Offices Rome"],alternates:{canonical,languages:{"it-IT":it,"en-GB":canonical,"x-default":it}},openGraph:{title:p.title,description:p.description,url:`${BASE_URL}${canonical}`,siteName:"Roma Office Sharing",locale:"en_GB",type:"website",images:[{url:"/images/office-hero.jpg",width:1200,height:627}]},twitter:{card:"summary_large_image",title:p.title,description:p.description,images:["/images/office-hero.jpg"]}}}
export default async function EnglishPage({params}:Props){const{slug}=await params;const{key}=resolvePath(["en",slug]);return <PageView pageKey={key} lang="en"/>}

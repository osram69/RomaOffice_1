...
See `docs/route-map.md` for all 20 Italian → English slug/title pairs and supplied-copy placement. All requested routes exist, including `/index.html` through the Italian dynamic route in addition to `/`.
...
1. `POST /api/order` creates a `pending` server-side record.
2. `POST /api/create-payment-session` creates the provider session. A verified webhook moves the record to `paid`; never trust a browser redirect as proof of payment.
3. The protected order form should require a random order token plus authenticated/emailed access link. On completion status becomes `filled`.
4. `POST /api/send-otp` issues a 10-minute code (Twilio when configured; `123456` only without credentials for local demonstration). `POST /api/verify-otp` hashes with HMAC, compares in constant time, and limits attempts.
5. Recommended: use DocuSign, Adobe Acrobat Sign, Yousign or SignRequest for an audit trail and provider-managed SMS verification. Store the provider envelope ID; process a verified `/api/webhook/esign`, download the final file server-to-server, then mark `signed`.
6. Self-hosted alternative: signature canvas → `POST /api/save-signature` → PDF with timestamp/order metadata. Canvas signatures may not provide the legal assurance required for a particular agreement. Obtain Italian legal advice and prefer a certified provider where enforceability matters.
7. Email the customer/admin an order summary and an expiring (5–15 minute), single-purpose signed download link. Show retry paths for SMS, signature and email failures.

The UI demonstrates steps and states. Before accepting real orders, add authentication to an `/order/[token]` area, bind all OTP/signature operations to a paid database record, add IP/device audit metadata under a documented lawful basis, and virus-scan uploaded documents.
